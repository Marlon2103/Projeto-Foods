...
describe('Reab1Etapa_Parte1 - Projeto BRF', () => {
  before(() => {
  ...
  it('Deve realizar login com sucesso', () => {
    ...
    cy.get('.space-y-3 > .w-full').should('be.visible').type(`${dados.TANUM}`) //Preenche o campo com o número da OT salva na variavel do TORD
    cy.wait(1000);
    cy.get('div.flex.items-center.gap-1.text-sm').contains('OT').click() //Clicar na tela para fechar o submenu
    cy.wait(3000).then(() => {
      cy.contains(`${dados.TANUM}`).should('exist');
    });
    cy.log(`✅ Check OK - OT ${dados.TANUM} validada com sucesso`)

    // Valida se o status "Pendente" está na mesma linha da OT
    cy.contains(`${dados.TANUM}`)
      .parents('tr, [role="row"], li, .flex, div')
      .first()
      .should('contain.text', 'Pendente');
    cy.log(`✅ Check OK - OT ${dados.TANUM} validada com status Pendente`)
